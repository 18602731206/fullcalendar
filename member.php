...
switch($action){
	...
	case "loginGo":  
		...
		//var_dump($_POST);		
		$sql="select * from member where username='{$_POST['username']}' and password='".md5($_POST['password'])."'";
		//echo $sql;
		...
		if($row['id']){
			...
			echo "<script>location='today.php'</script>";
		}
		else {echo "<script>alert('用户名或密码错误');location='index.html'</script>";}	
	break;
	...
	case "register":
		...
		<span class='status' style='font-size:20px;'>&nbsp;&nbsp;<i>会员<?php echo $M['id']?"编辑":"注册"; ?></i></span>&nbsp;&nbsp;&nbsp;&nbsp;
		 ...
		 <span class='myspan' style='width:70px;'>用户名：</span><input name='username'   style='width:150px;' value='<?php echo $M['username'] ?>'><br>
		 <span class='myspan' style='width:70px;'>密码：</span><input name='password' type='password' style='width:150px;'><br>
		 <span class='myspan' style='width:70px;'>姓名：</span><input name='realname'   style='width:150px;' value='<?php echo $M['realname'] ?>'><br>
		 <span class='myspan' style='width:70px;'>年龄：</span><input name='age'   style='width:150px;' value='<?php echo $M['age'] ?>'><br>
		 <span class='myspan' style='width:70px;'>性别：</span><select name='gender'   style='width:150px;'><option>男</option><option <?php echo select('女',$M['gender']) ?>>女</option></select><br>
