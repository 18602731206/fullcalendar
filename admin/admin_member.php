...
switch($action){
	case "insert":
		...
		<form action='?action=save&id=<?php echo $id?>' method='post'  onsubmit='return check()'  enctype='multipart/form-data' class='myform' style='width:420px;height:320px;'>
        ...
        <span class='myspan' style='width:70px;'>上传照片：</span><input type='file' style='width:220px;' id='upload' name='upload'><br>
	...
	case "alldel":
		...
		for($i=0;$i<count($key);$i++){
			query("delete from member where id={$key[$i]}"); 	    
		}
